...
const Images = forwardRef<HTMLDivElement, Props>(function Images(
    ...
    return (
        <div className='w-full p-5 mt-5 bg-white xl:mt-10 rounded-xl max-h-[80vh]'>
            <div
                ref={ref}
                className=' flex xl:block xl:h-[700px] xl:py-10  overflow-x-auto snap-x space-x-10 xl:space-x-0 xl:overflow-y-auto  snap-mandatory xl:snap-y xl:space-y-10 max-h-[70vh]'
            >
                {data?.map((image) => (
                    <div
                        key={image.id}
